...
const CreateRoomGroup = (props) => {
    ...
    function scheduleID() {
        ...
        return url.substring(0, n - 1) + `/roomGroup/${id}`;
    }
    ...
    return (
        <div>
            <section id="oo-lp">
                <Row>
                    <Col md={6}>
                        <img src={groupPage} className="d-none d-md-block about-img-cr"></img>
                    </Col>
                    <Col className="align-self-center welcomeCR" md={6}>
                        <h1>Group Video Call</h1>
                        <h1>with Bridge</h1>
                        <p>Join or Schedule a group video call <br></br> with your peers!</p>
                        
                        <button className="schedule" onClick={() => window.location.replace("/")}> Home </button>
                        <button className="schedule" onClick={() => setModalShow(true)}> Schedule Call </button>
                        <MyVerticallyCenteredModal show={modalShow} onHide={() => setModalShow(false)}/>
                        <button className="schedule" onClick = {create}> Instant Call </button>
                    </Col>
